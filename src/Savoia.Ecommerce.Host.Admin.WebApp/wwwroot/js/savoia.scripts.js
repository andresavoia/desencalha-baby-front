...
var Constantes = {
    ...
    ACAO_INCLUIR: "Incluir",
    ACAO_EXCLUIR: "Excluir",
    MENSAGEM_ERRO_VALIDACAO: 'Verifique os campos grifados em vermelho',
    MENSAGEM_REGISTRO_EXCLUIR : "Tem certeza que deseja excluir este registro?",
    CodNegociacaoStatusCancelado: 3,
    CodNegociacaoStatusQuitado: 2

}


var COD_SISTEMA_COMERCIAL = 1;
var COD_SISTEMA_LOTE = 2;
var COD_SISTEMA_ECOMMERCE = 3;
var COD_USUARIO_PERFIL_COM_ADM = "COM_ADM";
var COD_USUARIO_PERFIL_LOT_ADM = "LOT_ADM";
var COD_ESTADO_CIVIL_CASADO = 2;
var COD_ESTADO_CIVIL_SOCIEDADE = 5;
var COD_ESTADO_CIVIL_UNIAO_ESTAVEL = 7;
var ALTURA_PADRAO_DIMUICAO = 160;
...
$(window).load(function () {
    ...
    //Trocando o validationSummary por mensagem
    if ($("#erros ul li").css('display') != "none" && $("#erros ul li").val() != undefined)
        $('#ValidationSummaryModal').modal('show');

    //Trocando validator padrão por tooltip
    $(function () {
        $("form input, select").tooltipValidation({
            placement: "bottom",
            'container': 'body'
        });
    });
    ...
    $(document).ready(function () {
        ...
        if ($("#hdnScriptTemporario").val() != "") {
            eval($("#hdnScriptTemporario").val());
            $("#hdnScriptTemporario").val("");
        }

        //Coloca Tooltip em todos os componentes que utilizam no sistema inteiro
        $(function () {
...
$(".savoia-campo-valor").maskMoney({ allowZero: true, allowNegative: true, defaultZero: false, thousands:'.', decimal : ','});
    ...
    //{ reverse: true }
    //);
; //.mask('0.000.000,00', { reverse: true });
...
var Js = (function (Js) {
    ...
    var _cidadeAtual = ""
    Js.Funcao.CarregarCidade = function (codEstado, idCampoCidadeCod, idCampoCidadeDesc, itens) {

        var url = '/PessoaFisica/PesquisarCidadeEspecificaJson';
        url += "?CodEstado=" + codEstado;
    ...
    var _adquirenteAtual;
    Js.Funcao.CarregarAdquirente = function (idCampoCod, idCampoDesc, funcaoAposSelecionar, funcaoLimpar) {

        $("script[src^='typeahead']"); //Limpando o componente com versao anterior

        // Instantiate the Bloodhound suggestion engine
        var adquirentes = new Bloodhound({
            datumTokenizer: function (datum) {
                return Bloodhound.tokenizers.whitespace(datum.text);
            },
            queryTokenizer: Bloodhound.tokenizers.whitespace,
            remote: {
                url: '/Adquirente/PesquisarJson?totalRegistros=15&query=%QUERY',
                wildcard: '%QUERY',
                filter: function (adquirentes) {

                    // Map the remote source JSON array to a JavaScript object array
                    return $.map(adquirentes, function (adquirente) {
                        return {
                            text: adquirente.Nome,
        ...
        adquirentes.initialize();

        // Instantiate the Typeahead UI
        $('#' + idCampoDesc).typeahead(
            {
                minLength: 3,
                highlight: true,
                hint: false
            },
            {
                ...
                $("#" + idCampoCod).val(datum.value);
                _adquirenteAtual = datum.text;

                $(funcaoAposSelecionar);

            }).bind('typeahead:render', function () {
                _ajaxBloqueado = true;
            }
            ).bind('typeahead:close', function () {
                _ajaxBloqueado = false;
            });


        $('#' + idCampoDesc).focusin(function () {
            _adquirenteAtual = $(this).val();
        });

        $('#' + idCampoDesc).focusout(function () {

            if (_adquirenteAtual != $("#" + idCampoDesc).val()) {
...
;                $("#" + id).find("input, select").each(function () {
    ...
    Js.Util.Janela.MostrarMensagem = function (mensagem, funcaoBotaoOk,funcaoPosExecucao, opcoes) {
        ...
        BootstrapDialog.show({
            ...
            title: opcoes.titulo,
            message: "<div class='col-xs-3'><img src='/images/ico-modal-info-60x60.png' class='savoia-modal-icone'></div><div  class='col-xs-9'>" + opcoes.mensagem + "</div>",
            ...
            buttons: [{
                ...
                action: function (dialogItself) {
                    ...
                    if (funcaoPosExecucao != undefined && funcaoPosExecucao != "")
                        $(setTimeout(funcaoPosExecucao, 100));

                    if (funcaoBotaoOk != undefined && funcaoBotaoOk != "") {
                        $(setTimeout(funcaoBotaoOk, 100));
                    }
    ...
    Js.Util.Janela.MostrarMensagemErro = function (mensagem, functionPosExecucao, opcoes, funcaoBotaoOk) {
        ...
        BootstrapDialog.show({
            ...
            draggable: true,
            title: opcoes.titulo,
            message: "<div class='col-xs-3'><img src='/images/ico-modal-erro-60x60.png' class='savoia-modal-icone'></div><div  class='col-xs-9'>" + opcoes.mensagem + "</div>",
            type: BootstrapDialog.TYPE_DANGER,//BootstrapDialog.TYPE_DEFAULT,
            closable: false, // <-- Default value is false
            draggable: true, // <-- Default value is false
            buttons: [{
                label: '<span class="glyphicon glyphicon-ok"></span>  OK ',
                ...
                action: function (dialogItself) {
                    ...
                    if (functionPosExecucao != undefined && functionPosExecucao != "")
                        $(setTimeout(functionPosExecucao, 100));
    ...
    Js.Util.Janela.MostrarMensagemConfirmacao= function (mensagem, functionSim, functionNao, opcoes) {
        ...
        BootstrapDialog.show({
            ...
            title: opcoes.titulo,
            message: "<div class='col-xs-3'><img src='/images/ico-modal-pergunta-60x60.png' class='savoia-modal-icone'></div><div  class='col-xs-9'>" + opcoes.mensagem + "</div>",
            closable: false, // <-- Default value is false
            draggable: true, // <-- Default value is false
            buttons: [
            {
